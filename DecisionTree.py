...
def entropy(rows):
	...
	for lbl in counts:
		prob_of_lbl = counts[lbl] / float(len(rows))
		entropy_measure -= prob_of_lbl * math.log2(prob_of_lbl)
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.assigned_class= majority_class(self.predictions)
...
class Decision_Node:
    ...
    def get_id_left_child(self):
    	return 2 * self.id + 1

    def get_id_right_child(self):
    	return 2 * self.id + 2
...
def prune_tree(node, prunedList):
    ...
    #CS
def new_pruning(node):

    if isinstance(node, Leaf):
        return node
    elif node.id != 0:
        print (node.get_id_parent)
        return Leaf(node.rows, node.get_id_parent, node.depth - 1)

    node.true_branch = new_pruning(node.true_branch)
    node.false_branch = new_pruning(node.false_branch)
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        for item in node.predictions:
        	print (item, node.predictions[item])
        #CS	
        return
    ...
    #CS
    print ("Decision Node Depth" + spacing + str(node.depth))
    print ("Decision Node Id"	+ spacing +str(node.id))  
    #CS
...
def getPrunedList(node, p_list = []):

    if isinstance(node, Leaf):
        if not (node.id-1)/2 == 0:
            p_list.append((node.id - 1) / 2)
    else:
        getPrunedList(node.true_branch, p_list)
        getPrunedList(node.false_branch, p_list)    
    return p_list
